...
const heroTrialInfo = useMemo(() => buildHeroTrialInfo(restaurant), [restaurant])
...
return (
    ...
    <div className="owner-dashboard-hero">
        ...
        <button
            type="button"
            className={`owner-dashboard-trial-action ${heroTrialInfo.tone}`}
            onClick={() => onOpenSection?.('subscription-billing')}
        >
            <CreditCard size={16} />
            <span>{heroTrialInfo.kicker}</span>
            <strong>{heroTrialInfo.title}</strong>
            <small>{heroTrialInfo.cta}</small>
        </button>
...
function buildHeroTrialInfo(restaurant) {
const status = String(restaurant?.subscription_status || 'trialing').toLowerCase()
const plan = String(restaurant?.subscription_plan || 'trial').replace(/[_-]+/g, ' ')
const isActive = ['active', 'paid', 'subscribed'].includes(status)
const targetDate = isActive
    ? getHeroDateFromRestaurant(restaurant, [
    ...
    : getHeroDateFromRestaurant(restaurant, [
        ...
        'trial_until',
    ]) || getHeroFallbackTrialEnd(restaurant)

const remaining = getHeroRemainingTime(targetDate)
...
if (isActive) {
    ...
    kicker: 'Subscription',
    title: targetDate
        ? `${formatTitle(plan)} active • ${remaining.label}`
        : `${formatTitle(plan)} active`,
    cta: 'Manage Plan',
    }
}
...
if (remaining.expired) {
    ...
    tone: 'expired',
    kicker: 'Trial ended',
    title: 'Subscribe to keep Spizy active',
    cta: 'Subscribe Now',
    }
}
...
return {
    ...
    title: `${remaining.label} left`,
    cta: 'Subscribe Now',
}
}

function getHeroDateFromRestaurant(restaurant, keys) {
for (const key of keys) {
...
function getHeroFallbackTrialEnd(restaurant) {
...
if (Number.isNaN(createdDate.getTime())) {
    return addHeroDays(new Date(), 3)
}

return addHeroDays(createdDate, 14)
}

function addHeroDays(date, days) {
const nextDate = new Date(date)
nextDate.setDate(nextDate.getDate() + days)
return nextDate
}

function getHeroRemainingTime(targetDate) {
if (!targetDate) return { expired: true, label: '0d 0h' }

const diff = targetDate.getTime() - Date.now()
if (diff <= 0) return { expired: true, label: '0d 0h' }
...
return { expired: false, label: `${hours}h ${minutes}m` }
}
