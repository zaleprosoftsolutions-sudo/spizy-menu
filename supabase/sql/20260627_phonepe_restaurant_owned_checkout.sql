-- Spizy Menu - PhonePe restaurant-owned checkout foundation
-- Each restaurant connects and uses its OWN PhonePe PG account. Spizy/Zalepro does not process restaurant customer payments through a shared PhonePe account.
...
begin
  ...
  set payment_gateway_settings = jsonb_set(
    ...
    '{phonepe}',
    coalesce(payment_gateway_settings->'phonepe', '{}'::jsonb) || jsonb_build_object(
      'enabled', coalesce((payment_gateway_settings->'phonepe'->>'enabled')::boolean, false),
      'test_mode', coalesce((payment_gateway_settings->'phonepe'->>'test_mode')::boolean, true),
      'connection_status', coalesce(payment_gateway_settings->'phonepe'->>'connection_status', 'not_connected'),
      'credential_status', coalesce(payment_gateway_settings->'phonepe'->>'credential_status', 'missing'),
      'checkout_mode', coalesce(payment_gateway_settings->'phonepe'->>'checkout_mode', 'redirect')
    ),
    true
  )
...
exception
  when others then
    raise notice 'PhonePe payment_gateway_settings backfill skipped: %', sqlerrm;
end $$;
