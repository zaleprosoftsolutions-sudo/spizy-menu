-- Spizy Menu - Cashfree restaurant-owned checkout foundation
-- Each restaurant connects and uses its OWN Cashfree account. Spizy/Zalepro does not process restaurant customer payments through a shared Cashfree account.
...
begin
  ...
  set payment_gateway_settings = jsonb_set(
    ...
    '{cashfree}',
    coalesce(payment_gateway_settings->'cashfree', '{}'::jsonb) || jsonb_build_object(
      'enabled', coalesce((payment_gateway_settings->'cashfree'->>'enabled')::boolean, false),
      'test_mode', coalesce((payment_gateway_settings->'cashfree'->>'test_mode')::boolean, true),
      'connection_status', coalesce(payment_gateway_settings->'cashfree'->>'connection_status', 'not_connected'),
      'credential_status', coalesce(payment_gateway_settings->'cashfree'->>'credential_status', 'missing'),
      'checkout_mode', coalesce(payment_gateway_settings->'cashfree'->>'checkout_mode', 'payment_link')
    ),
    true
  )
...
exception
  when others then
    raise notice 'Cashfree payment_gateway_settings backfill skipped: %', sqlerrm;
end $$;
