-- Spizy Menu — Network International / N-Genius restaurant-owned gateway foundation
-- Each restaurant connects its own Network / N-Genius merchant credentials.
-- Spizy/Zalepro does not use a shared Network merchant account for restaurant customer payments.
...
comment on column public.restaurant_gateway_credentials.public_key is
  'For Network / N-Genius this stores the restaurant Outlet Reference. For Razorpay it stores Key ID; for Cashfree/PhonePe it stores Client ID.';

comment on column public.restaurant_gateway_credentials.access_token is
  'For Network / N-Genius this stores the restaurant Hosted Payment Page API Key. For other gateways it stores that restaurant-owned secret credential.';
