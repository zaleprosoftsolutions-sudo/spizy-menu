-- Spizy Menu - Razorpay restaurant-owned checkout foundation
-- Each restaurant connects and uses its OWN Razorpay account. Spizy/Zalepro does not process restaurant customer payments through a shared Razorpay account.
...
begin
  ...
  set payment_gateway_settings = jsonb_set(
    ...
    '{razorpay}',
    coalesce(payment_gateway_settings->'razorpay', '{}'::jsonb) || jsonb_build_object(
      'enabled', coalesce((payment_gateway_settings->'razorpay'->>'enabled')::boolean, false),
      'test_mode', coalesce((payment_gateway_settings->'razorpay'->>'test_mode')::boolean, true),
      'connection_status', coalesce(payment_gateway_settings->'razorpay'->>'connection_status', 'not_connected'),
      'credential_status', coalesce(payment_gateway_settings->'razorpay'->>'credential_status', 'missing'),
      'checkout_mode', coalesce(payment_gateway_settings->'razorpay'->>'checkout_mode', 'payment_link')
    ),
    true
  )
...
exception
  when others then
    raise notice 'Razorpay payment_gateway_settings backfill skipped: %', sqlerrm;
end $$;
