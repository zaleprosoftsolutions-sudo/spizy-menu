...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (userError || !user) {
      return jsonResponse({ error: 'Login required to verify subscription payment.' }, 401)
    }
    ...
    if (!hasAccess) {
      return jsonResponse({ error: 'You do not have permission to verify this restaurant subscription.' }, 403)
    }
    ...
    if (attemptId) {
      ...
      query = query.eq('mamo_link_id', paymentLinkId)
    } else if (transactionId) {
      query = query.eq('mamo_transaction_id', transactionId)
    } else {
      return jsonResponse({ error: 'attempt_id, payment_link_id or transaction_id is required.' }, 400)
    }
    ...
    if (attemptError || !attempt) {
      return jsonResponse({ error: attemptError?.message || 'Subscription payment attempt not found.' }, 404)
    }
    ...
    if (transactionId && mamoApiKey) {
      ...
      chargeStatus = normalizeStatus(
        extractFirstString(chargeResponse, ['status', 'payment_status', 'charge_status', 'state']),
      )
    }
    ...
    const periodEnd = updatedAttempt.billing_period_end || addDaysDateKey(new Date(), 30)
    const graceUntil = updatedAttempt.grace_until || addDaysDateKey(new Date(), 37)
    ...
    if (!existingInvoice) {
      await adminClient.from('restaurant_subscription_invoices').insert({
...
function normalizeStatus(value: string) {
  ...
  if (['captured', 'paid', 'succeeded', 'success', 'settled', 'completed'].includes(normalized)) {
    return 'captured'
  }
